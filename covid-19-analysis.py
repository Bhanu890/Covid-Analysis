...
most_affected_c=total_cases[1:16].index
cases=total_cases[1:16].values
...
def countryCompare():
    ...
    plt.xticks([datewise_cases['date'][i] if i%15==0 else '' for i in range(len(datewise_cases.index))],rotation=60)
    plt.legend()
    plt.xlabel("Date");
...
def factor():
    ...
    df_corr['Diabetes']=pop_density[most_affected_c].values
    
    stringency_index=df.groupby('location')['stringency_index'].nth(-1)
    df_corr['Diabetes']=median_age[most_affected_c].values
    ...
    heat=pd.DataFrame({"Correlation Values":df_corr.corr()['Total Cases'].values},index=df_corr.corr()['Total Cases'].index)
...
b1.pack(pady=5)
b1=Button(frame,text="Top Ten Testing" ,command=topTenTesting)
b1.pack(pady=5)
b1=Button(frame,text="Comparison between two countries" ,command=countryCompare)
b1.pack(pady=5)
b1=Button(frame,text="Factors" ,command=factor)
b1.pack(pady=5)
frame.mainloop()
